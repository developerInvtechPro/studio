...
export async function getCustomersAction(): Promise<Customer[]> {
    try {
        const db = await getDbConnection();
        return db.all<Customer[]>("SELECT * FROM customers ORDER BY name");
    } catch (error) {
...
export async function saveCustomerAction(customer: Omit<Customer, 'id'> & { id?: number }): Promise<{ success: boolean; data?: Customer; error?: string }> {
    try {
        const db = await getDbConnection();
        const { id, name, rtn, phone, address } = customer;
        ...
        if (rtn) {
             const existingCustomer = await db.get<Customer>("SELECT * FROM customers WHERE rtn = ? AND id != ?", rtn, id || 0);
             if (existingCustomer) {
                 return { success: false, error: "El RTN ya está registrado para otro cliente." };
             }
        }
        ...
        if (id) {
            await db.run(
                `UPDATE customers SET name = ?, rtn = ?, phone = ?, address = ? WHERE id = ?`,
                name, rtn, phone, address, id
            );
        } else {
            const result = await db.run(
                `INSERT INTO customers (name, rtn, phone, address) VALUES (?, ?, ?, ?)`,
        ...
        if (!lastId) {
            return { success: false, error: 'Could not determine customer ID after save.' };
        }

        const savedCustomer = await db.get<Customer>("SELECT * FROM customers WHERE id = ?", lastId);
        
        if (!savedCustomer) {
            return { success: false, error: 'Could not retrieve customer after saving.' };
        }
