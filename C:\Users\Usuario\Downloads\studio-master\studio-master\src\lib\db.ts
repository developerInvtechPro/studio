...
const initializeDb = async () => {
    const db = await open({
        filename: './cafe_central_v11.db', // <--- Incremented DB version
        driver: sqlite3.verbose().Database,
    });

    await db.exec('PRAGMA foreign_keys = ON;');
    
    // Check for stock column and add if it doesn't exist
    const productsTableInfo = await db.all("PRAGMA table_info(products);");
    if (!productsTableInfo.some(col => col.name === 'stock')) {
        await db.exec('ALTER TABLE products ADD COLUMN stock REAL NOT NULL DEFAULT 0;');
    }
